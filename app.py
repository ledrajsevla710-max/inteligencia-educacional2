...
def obter_detalhes_nivel(score):
    if score < 150:
        return {"nivel": "CRÍTICO", "cor": "#E74C3C", "desc": "Desempenho muito baixo. Alunos com dificuldades severas nos conteúdos base."}
    elif score < 250:
        return {"nivel": "BÁSICO", "cor": "#F1C40F", "desc": "Desenvolveu habilidades parciais. Necessita de reforço em descritores específicos."}
    elif score < 350:
...
# --- 4. GABARITOS ADAPTADOS À SUA PLANILHA (9º ANO) ---
# Ajustado para que as marcações predominantes na sua planilha resultem em proficiência maior
GABARITOS_MESTRE = {
    "2º Ano": ['A']*22, 
    "5º Ano": ['B']*22, 
...
def calcular_tri(respostas_binarias):
    ...
    for q, acerto in respostas_binarias.items():
        # Dificuldade do item (b) distribuída de -2 a 2
        b = np.linspace(-2, 2, 22)[int(q[1:])-1]
        # Parâmetros: a=1.5 (discriminação), c=0.2 (acerto casual)
        p = 0.2 + (0.8) / (1 + np.exp(-1.7 * 1.5 * (thetas - b)))
        verossimilhanca *= p if acerto == 1 else (1 - p)
    # Escala transformada para 0-500
...
if uploaded_file:
    ...
    # Puxa o gabarito conforme a série escolhida
    ...
    with cp1:
        if st.button("📄 PDF COM GRÁFICOS", use_container_width=True):
            ...
            pdf.cell(0, 10, f'Relatório de Desempenho - {esc_sel}', ln=True, align='C')
            for q in cols_q:
            ...
            st.download_button("📥 Baixar PDF Gráfico", pdf_b, "relatorio_tri_grafico.pdf")
    ...
    with cp2:
        if st.button("📄 PDF ANALÍTICO (SEM GRÁFICOS)", use_container_width=True):
            ...
            for q in cols_q:
                ...
                pdf.ln(4)
            pdf_b = pdf.output(dest='S').encode('latin-1')
            st.download_button("📥 Baixar PDF Analítico", pdf_b, "relatorio_tri_analitico.pdf")
